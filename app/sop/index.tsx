...
export default function SOPScreen() {
  const sops = [{ id: '1', title: '地基開挖作業標準', category: '基礎工程', date: '2023-12-01' }];
  ...
  return (
    <View style={{flex: 1, backgroundColor: '#F5F7FA'}}>
      ...
      <FlatList 
        data={sops}
        renderItem={({item}) => (
          <TouchableOpacity style={styles.card} onPress={() => Alert.alert('SOP 查看', `開啟文件：${item.title}`)}>
            <Ionicons name="document-text" size={24} color="#C69C6D" />
            <View style={{marginLeft: 15, flex: 1}}><Text style={{fontSize: 16, fontWeight:'bold'}}>{item.title}</Text><Text style={{color:'#999'}}>{item.category}</Text></View>
            <Ionicons name="chevron-forward" size={20} color="#ccc" />
          </TouchableOpacity>
        )}
      />
      <TouchableOpacity style={styles.fab} onPress={() => Alert.alert('新增資料', '開啟上傳 SOP 視窗')}>
